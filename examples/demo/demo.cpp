...
void PlayDemo(int argc, char** argv)
{
	...
	try
	{
		...
		*/
		...
		// create an encoder - this encoder will receive raw data from any src (filter, file, container, memory, etc),
		...
		*/
		...
		// this encoder will receive data from an input src (file, raw, filter, etc), encode it and send it to the output container (muxer)
		...
		*/
		...
		if (audioEncoder != nullptr)
		{
			...
			{
				// if the input comes from a container, we use the demuxer class - it is just an input src like any other
		...
		*/
		...
		// Instead of feeding the video src directly to the encoder, we feed it to
		...
		if (videoFilterConfig != NULL && videoEncoder != nullptr)
		{
			...
			videoFrameSink = videoFilter; // used to feed the src below
		}
		...
		*/
		...
		// As an awkward but necessary first step, we need to prime each input src.
